...
class MyWebSocket extends EventEmitter {
  constructor(server) {
    super();
    // 这里我们就先默认用户肯定先传一个服务地址过来
    // 第一步有了一个node服务
    this.server = server;
    // 第二步有了自己的web服务  现在要通过http协议将我们服务升级为一个ws协议  浏览器开始升级  ws: xxxxx
    // 将我们自己的服务升级到 ws协议服务  服务会响应一个101的状态
    this.server.on('upgrade', (req, socket, upgradeHead) => {
      ...
      var responseHead = [
          ...
          hashWebSocketKey(req.headers['sec-websocket-key']),
        ]
        .concat('', '')
        .join('\r\n');

      socket.write(responseHead);
      // 第三步  上面建立了连接  接受客户端发送的消息

      // this.keepLive()
      socket.on('data', (data) => { //  注意这里得到的是一个buffer数据
  ...
  keepLive(time = 2000) {
    ...
    function s() {
      that.doSend(OPCODES.PING, Buffer.from('ping'));
      console.log('server send ping...');
      // 在关闭连接的情况下就不再需要发送 ping 请求了
      setTimeout(s, time);
    }
    s()
    // this.doSend(OPCODES.PING, Buffer.from('ping'));
